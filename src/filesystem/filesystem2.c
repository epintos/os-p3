...
void getCPParams(char * input, char ** params);
void getLinkParams(char * input, char ** params);
...
//	cleanDisk();
	...
	if (existsFilesystem()) {
		//		printf("Filesystem exists. Please wait while its being loaded...\n");
...
int create(char* pathname, char* newPath, int mode, int type) {
	...
	while (pathname[i] != '\0' && flag) {
		...
		if (actual->type == LINK)
			actual = getHeaderFromLink(actual);
	...
	if (!flag) {
		...
		case LINK:
			...
			return createLink(name, newPath, mode, prev);
		}
...
int mkfifo(char* pathname, int mode) {
	...
	return create(pathname, NULL, mode, FIFO);
}

int creat(char* pathname, int mode) {

	return create(pathname, NULL, mode, FILE);
}

int mkdir(char* pathname) {

	return create(pathname, NULL, DIR_PERMS, DIR);
}
...
generalHeader* getHeader(generalHeader* parent, char* name) {
	...
	for (i = 0; k < parent->cantFiles && cantSects < 8; i++, pos++, k++) {
		if (i % FILES_PER_SECTOR == 0) {
			diskRead((char*) files, sizeof(file) * 8, (parent->dataSect)
					+ cantSects, 0);
			cantSects++;
			pos = 0;
		}
		if (files[pos].numInode == -1) {
			...
			aux = searchHeader(files[pos].numSect, files[pos].numInode);
...
generalHeader* getHeaderFromLink(generalHeader* link) {
	fragSector sector;
	generalHeader* resp;
	diskRead((char*) &sector, SECTOR_SIZE, link->dataSect, 0);
	resp = searchPath(sector.info);
...
void createNewHeader(generalHeader* newHeader, char* name, generalHeader* prev,
	...
	if (type == LINK)
		writeLinkPath(newHeader->dataSect, name);
	writeHeaderToDisk(newHeader);
	return;
}

void writeLinkPath(int dataSect, char* linkPath) {
	fragSector sect;
	sect.nextSect = -1;
	sect.sectQty = 7;
	strcpy(linkPath, sect.info);
...
int readFromFile(int pos, char* buff, int size) {
	...
	for (i = 0; i < size; i++) {
		...
		if (table.openFiles[pos].header.data[table.openFiles[pos].pos] == '\0') {
			buff[i] = '\0';
			return i;
		} else {
			buff[i]
...
void deleteChilds(generalHeader* deleteFrom) {
	...
	for (i = 0; k < deleteFrom->cantFiles && cantSects < 8; i++, pos++, k++) {
		...
		if (files[pos].numInode == -1) {
			k--;
		} else if (!strcmp(".", files[pos].name) && !strcmp("..",
				files[pos].name)) {
...
void delet(generalHeader** start) {
	int back = kernel;
	kernel = 1;
	//	printf("Los Headers que devuelve StartPoint son : \n");
	//	printHeader(start[0]);
	//	printHeader(start[1]);
	kernel = back;
...
generalHeader** startPoint(char* pathname) {
	...
	while (pathname[i] != '\0' && flag) {
		...
		if (resp[0]->type == LINK && pathname[i] != '\0') {
			resp[0] = getHeaderFromLink(resp[0]);
		}
		if (resp[0] == NULL) {
			flag = 0;
...
int link(int unused, char** paths) {
	char** parsed = (char**) malloc(2 * sizeof(char));
	getLinkParams(paths[0], parsed);
	char * pathname = parsed[0];
	char * linkname = parsed[1];
	generalHeader* old;
	old = searchPath(pathname);
	int back = kernel;
	kernel = 1;
	kernel = back;
	if (old == NULL) {
		//		printf(" SEARCH PATH = NULL\n");
		return -1;
	}

	return create(linkname, pathname, old->permissions, LINK);

}

int createLink(char* pathname, char* newPath, int mode, generalHeader* prev) {
	generalHeader newLink;
	createNewHeader(&newLink, newPath, prev, LINK, mode);

	createNewEntry(pathname, newLink.inodeNumber, newLink.numSect, prev);

	refreshSuperBlock();
	return 1;
}

int mv(int unused, char** argv) {
	char ** parsed = malloc(sizeof(char) * 2);
	getCPParams(argv[0], parsed);
	char * oldPath = parsed[0];
	char * newPath = parsed[1];
	generalHeader** start = startPoint(oldPath);
	generalHeader* where = searchPath(newPath);
	char name[MAX_NAME_LENGHT];
	int i = 0;
	if (where == NULL) {
		printf("No existe archivo destino\n");
		return 0;
	}
	if (start == NULL || where->type != DIR) {
		printf("No existe archivo origen\n");
		return 0;
	}

	deleteEntry(start[1], start[0]->inodeNumber);
	while (oldPath[i] != '\0') {
		getNameFromPath(oldPath, &i, name);
		int back = kernel;
		kernel = 1;
		printf("%s\n", name);
		kernel = back;
	...
	if (start[0]->type == DIR) {
		createFirstFiles(start[0]->dataSect, start[0]->numSect,
				start[0]->inodeNumber, where->numSect, where->inodeNumber);
	}
	createNewEntry(name, start[0]->inodeNumber, start[0]->numSect, where);

	return 1;
}

int cp(int unused, char** paths) {
	char ** parsed = malloc(2 * sizeof(char));
	getCPParams(paths[0], parsed);
	...
	char name[MAX_NAME_LENGHT];
	int i = 0;
	generalHeader * origin = searchPath(oldPath);
	generalHeader * destiny = searchPath(newPath);
	if (origin == NULL || destiny == NULL || destiny->type != DIR) {
		return 0;
	}
	while (oldPath[i] != '\0')
		getNameFromPath(oldPath, &i, name);

	recursiveCopy(origin, destiny, name);
	return 0;

}

void recursiveCopy(generalHeader* origin, generalHeader* destiny, char* name) {
	if (origin->type != DIR) {
		copyFiles(origin, destiny, name);
		return;
	}

	copyDir(origin, destiny, name);
}

void copyFiles(generalHeader* origin, generalHeader* destiny, char* name) {
	fragSector sect;
	diskRead((char*) &sect, SECTOR_SIZE, origin->dataSect, 0);

	switch (origin->type) {
	case FILE:
		copyFile(name, origin->permissions, destiny, origin->dataSect);

		//copyDataFromFile(destiny, name, origin->dataSect);
		break;

	case FIFO:
		createFifo(name, destiny, origin->permissions);
		break;

	case LINK:
		createLink(name, sect.info, origin->permissions, destiny);
		break;
	}
}

void copyDir(generalHeader* origin, generalHeader* destiny, char* name) {
	generalHeader new;
	int i = 0;
	for (i = 0; i < forCopy.inodeQty; i++) {
		if (strcmp(forCopy.dirs[i], name))
			return;
	}

	createNewHeader(&new, name, destiny, DIR, DIR_PERMS);
	strcpy(name, forCopy.dirs[forCopy.inodeQty]);
	forCopy.inodeQty++;

	if (new.dataSect == -1)
		return;

	createNewEntry(name, new.inodeNumber, new.numSect, destiny);

	createFirstFiles(new.dataSect, new.numSect, new.inodeNumber,
			destiny->numSect, destiny->inodeNumber);

	refreshSuperBlock();

	copyEntries(origin, &new);
	return;

}

void copyEntries(generalHeader* origin, generalHeader* destiny) {
	...
	int pos = 0;
	int k = 0;
	for (i = 0; k < origin->cantFiles && cantSects < 8; i++, pos++, k++) {
		if (i % FILES_PER_SECTOR == 0) {
			diskRead((char*) files, sizeof(file) * 8, (origin->dataSect)
					+ cantSects, 0);
			cantSects++;
			pos = 0;
		}
		if (files[pos].numInode == -1) {
			...
			recursiveCopy(aux, destiny, files[pos].name);
		}
	}
	return;
...
void copyFile(char* name, int mode, generalHeader* parent, int originDataSect) {

	generalHeader newFile;
	createNewHeader(&newFile, name, parent, FILE, mode);

	createNewEntry(name, newFile.inodeNumber, newFile.numSect, parent);
	copyDataFromFile(newFile.dataSect, originDataSect);

	refreshSuperBlock();

}

void copyDataFromFile(int newData, int oldData) {
	fragSector sect, sect2;

	do {
		diskRead((char*) &sect, SECTOR_SIZE, oldData, 0);
		if (sect.nextSect != -1) {
			sect2.nextSect = getFreeBlocks();
			oldData = sect2.nextSect;
			newData = sect.nextSect;
		} else
			sect2.nextSect = -1;
		sect2 .sectQty = 7;
		diskWrite((char*) &sect, sizeof(int) * 2, newData, 0);

		diskWrite(sect.info, sizeof(sect.info), newData, sizeof(int) * 2);

	} while (sect.nextSect != -1);
}

void getCPParams(char * input, char ** params) {
	int index = 2;
	int i = 0, j = 0;
	params[0] = malloc(MAX_BUFFER);
	params[1] = malloc(MAX_BUFFER);

	while (input[index] == ' ')
		index++;

	while (input[index] != ' ') {
		params[0][i++] = input[index++];
	}
