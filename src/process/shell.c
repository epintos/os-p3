...
int shell(int argc, char ** argv) {
	...
	while (1) {
		...
		} else if (strcmp(c + back, "processE")) {
			pid = newProcess(procE, 0, 0, "ProcessE");
		...
		} else if (strcmp(c + back, "enableCache")) {
			enableCache();
			printf("Cache enabled\n");
		} else if (strcmp(c + back, "disableCache")) {
			disableCache();
		...
		} else if (startsWith(&((c + back)[0]), 2, "cp")) {
			char ** send = (char **) malloc(1 * sizeof(char*));
			send[0] = c;
			pid = newProcess(cp, 0, send, "cp");
		} else if (startsWith(&((c + back)[0]), 2, "mv")) {
			char ** send = (char **) malloc(1 * sizeof(char*));
			send[0] = c;
			pid = newProcess(mv, 0, send, "mv");
		} else if (startsWith(&((c + back)[0]), 4, "link")) {
			char ** send = (char **) malloc(1 * sizeof(char*));
			send[0] = c;
			pid = newProcess(link, 0, send, "link");
		} else if (!strcmp(c, "")) {
