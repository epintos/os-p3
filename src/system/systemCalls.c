...
void int_80(params * p) {
	...
	if (p->operation == WRITE) {
		...
		handleGetSem(p);
	} else if (p->operation == MALLOC) {
		mallocHandler(p);
	} else if (p->operation == REALLOC) {
		reallocHandler(p);
	} else if (p->operation == FREE) {
		freeHandler(p);
	} else {
...
void mallocHandler(params * p) {
	p->ret = Malloc(p->param2);
}

void reallocHandler(params * p) {
	p->ret = Realloc(p->ret, p->param2, *(p->param1));
}

void freeHandler(params * p) {
	Free(p->ret);
}
