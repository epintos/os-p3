...
uint _getNewContext() {
	...
	if (tNext == 99)
		tNext = 0;
	actual->st = RUNNING;
	if (checkStackPosition(actual->esp, actual->pid)) {
		krealloc(actual->pid);
	}
	upPages(actual->pid);
...
int createProcess(int(*process)(int, char**), int argc, char **argv,
	...
	all[rInsert].stack = kmalloc(pid);
	all[rInsert].esp = (int) all[rInsert].stack;
	if (strcmp(name, "ProcessG")) {
		printf("Stack para processG = %x\n", all[rInsert].stack);
	}
...
void createIDLE(int(*process)(int, char**), int argc, char **argv) {
	...
	all[0].name = "IDLE";
	all[0].st = READY;
	all[0].stack = kmalloc(0);
	all[0].esp = (int) all[0].stack;
...
void kill(char option, int signal, int pid) {
	...
	for (i = rNext; i < rInsert; i++) {
		if (all[i].pid == pid || all[i].parentPID == pid) {
			if (strcmp(all[i].name, "Shell") || strcmp(all[i].name, "LogIn"))
				return;
			all[i].st = DEAD;
			time = 0;
			notifyEndOfProcess(pid);
...
int getStackStart() {
	return actual->stackStart;
}

void setStackStart(int start) {
	actual->stackStart = start;
}

int getStackEnd() {
	return actual->stackEnd;
}

void setStackEnd(int end) {
	actual->stackEnd = end;
}

unsigned int _GetTemporaryESP() {
